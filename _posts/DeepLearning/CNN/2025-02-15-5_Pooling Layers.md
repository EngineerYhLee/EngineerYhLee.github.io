...
## 1. 풀링의 정의와 목적
풀링은 컨볼루션 신경망에서 **특징 맵(feature map)의 공간적 크기를 줄이는 다운샘플링 연산**입니다. 주요 목적은 다음과 같습니다:
...
- **연산량 감소:** 풀링 계층은 특징 맵의 크기를 줄여, 이후 컨볼루션이나 완전연결층에서 처리해야 할 데이터 양을 현저히 감소시켜줍니다.
...
- **글로벌 풀링 (Global Pooling):**  
  특징 맵 전체에 대해 하나의 값(예: 평균 또는 최대)을 산출하여, 완전연결층의 입력을 줄이고 모델 파라미터를 감소시킵니다.
